	...
	if (JSON.stringify(treeA_1[0]) === JSON.stringify(treeB_1[0]) && 
		...
		if (treeA_1[0] === 'eval') {
			...
			for (var [k, v] of treeA_1.entries()) {
                ...
                if (v[0] == 'equation' && treeB_1[k][0] != 'equation') {
                    return false;
                }
                ...
                var BReSci;
                if (v[0] == 'equation') {
                    AReSci = (parseFloat(v[1][0]) - parseFloat(v[2][0])).toExponential(5);
                    BReSci = (parseFloat(treeB_1[k][1][0]) - parseFloat(treeB_1[k][2][0])).toExponential(5);

                    AImSci = (parseFloat(v[1][1]) - parseFloat(v[2][1])).toExponential(5);
                    BImSci = (parseFloat(treeB_1[k][1][1]) - parseFloat(treeB_1[k][2][1])).toExponential(5);
                    ...
                    // echo '<pre>'.print_r([ $AReSci, $BReSci, $AImSci, $BImSci ], true).'</pre>';
                } else {
                    // 150818 larwein - 유효숫자 범위 줄임
                    AReSci = parseFloat(v[0]).toExponential(5);
                    BReSci = parseFloat(treeB_1[k][0]).toExponential(5);

                    AImSci = parseFloat(v[1]).toExponential(5);
                    BImSci = parseFloat(treeB_1[k][1]).toExponential(5);
                }

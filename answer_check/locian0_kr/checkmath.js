...
export function LatexToTree (A, node = null, node_idx = null) {
  let node_1 = JSON.parse(JSON.stringify(node))
  const partL = '@'
  const partR = '#'
...
function sub_LatexToTree (newA, node, node_idx) {
  const node_1 = JSON.parse(JSON.stringify(node))
  const partL = '@'
  const partR = '#'
  const delimeter = '\\'
  ...
  if (arr.length !== 1) {
    result = ['ratio']
    for (var v of arr) { result.push(sub_LatexToTree(v, node_1, node_idx)) }
    return result
  }
...
export function compareMathTree (treeA, treeB) {
  let treeA_1 = JSON.parse(JSON.stringify(treeA))
  let treeB_1 = JSON.parse(JSON.stringify(treeB))
...
export function is_equal_tree (tree_1, tree_2) {
  const tree_11 = JSON.parse(JSON.stringify(tree_1))
  const tree_21 = JSON.parse(JSON.stringify(tree_2))

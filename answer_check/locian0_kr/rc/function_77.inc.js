...
export function fracSimpVar(tree) {
    ...
    const newVarNum = varNum.filter(vn => !varDen.includes(vn));
    const newVarDen = varDen.filter(vd => !varNum.includes(vd));
    ...
    if (newVarNum_length !== 0 && newVarDen_length !== 0) {
        ...
        for (let k = numk; k < newVarNum_length; k++) {
            for (let i = deni; i < newVarDen_length; i++) {
                if (newVarNum[k][1][1] < newVarDen[i][1][1]) {
                    newNum = newVarNum[k][2][1] === '1'
                        ? [...newNum, newVarNum[k][1]]
                        : [...newNum, newVarNum[k]];
                    numk++;
                    if (numk === newVarNum.length) {
                        for (let j = deni; j < newVarDen_length; j++) {
                            newDen = newVarDen[j][2][1] === '1'
                                ? [...newDen, newVarDen[j][1]]
                                : [...newDen, newVarDen[j]];
                        }
                    }
                    break;
                } else if (newVarNum[k][1][1] === newVarDen[i][1][1]) {
                    const expo = newVarNum[k][2][1] - newVarDen[i][2][1];
                    if (expo > 0) {
                        newNum = expo === 1
                            ? [...newNum, ['variable', newVarNum[k][1][1]]]
                            : [...newNum, ['power', ['variable', newVarNum[k][1][1]], ['natural', expo.toString()]]];
                    } else if (expo < 0) {
                        newDen = expo === -1
                            ? [...newDen, ['variable', newVarDen[i][1][1]]]
                    ...
                    if (deni === newVarDen.length) {
                        for (let j = numk; j < newVarNum_length; j++) {
                            newNum = newVarNum[j][2][1] === '1'
                                ? [...newNum, newVarNum[j][1]]
                                : [...newNum, newVarNum[j]];
                        }
                    }
                    break;
                } else if (newVarNum[k][1][1] > newVarDen[i][1][1]) {
                    newDen = newVarDen[i][2][1] === '1'
                        ? [...newDen, newVarDen[i][1]]
                        : [...newDen, newVarDen[i]];
                    deni++;
                    if (deni === newVarDen.length) {
                        for (let j = numk; j < newVarNum_length; j++) {
        ...
        const newOperand = arrNum_length === 0
            ? [['natural', '1']]
            : arrNum_length === 1
                ? [arrNum[0][1]]
                : num_key === 0
        ...
        return arrDen_length === 0
            ? newOperand[0]
            : arrDen_length === 1
                ? [operator, ...newOperand, arrDen[0][1]]
                : den_key === 0
    ...
    const newNum = newVarNum.map(nn => nn[2][1] === '1'
        ? nn[1]
        : nn);
    ...
    const newOperand = arrNum_length === 0
        ...
        : arrNum_length === 1
            ? [arrNum[0][1]]
            : [mulCommutative(['mulchain', ...arrNum])];
    const newDen = newVarDen.map(nn => nn[2][1] === '1'
        ? nn[1]
        : nn);
    let arrDen = [];
    if (newDen.length > 0) {

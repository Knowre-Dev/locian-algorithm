...
export function exprSimpConst (tree = null) {
  ...
  switch (operator) {
    ...
    case 'fraction': // fin
      ...
      const numer = exprSimpConst(tree_1[0])
      const denom = exprSimpConst(tree_1[1])
    ...
    case 'power':
      ...
      if (expotree[0] === 'natural' && oddExpoFlag && evenRootFlag) { basetree = ['absolute', basetree] }
      ...
      if (basetree[0] === 'mulchain') {
        ...
        for (const term of basetree_1) {
          const subtree = exprSimpConst(['power', term[1], expotree])

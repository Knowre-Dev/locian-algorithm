    ...
    switch (tree[0]) {
        ...
        case 'inequality':
            ...
            if (tree[2] === 'gt' || tree[2] === 'ge') {
                ...
                let tree_2 = tree.slice(1).reverse();
                for (let [k, subtree] of tree_2.entries()) {
                    k === tree.length - 2 ? newOperands.push(['natural', '0'])
        ...
        default:
            ...
            for (let seed of seedArr) {
                let evalresult = evaluateExWithSeed(tree, seed);
...
export function evaluateOperation(operator, operand, seed = null, lookupTable = new Object()) {
    ...
    switch (operator) {
        ...
        case 'div':
            ...
            let modulusSq = Math.pow(operand[0][0], 2) + Math.pow(operand[0][1], 2);
        ...
        case 'fraction':
            ...
            let numerRe = operand[0][0];
            let numerIm = operand[0][1];
            let denomRe = operand[1][0];
            let denomIm = operand[1][1];
        ...
        case 'nthroot':
            ...
            // z2^(z1*/|z1|^2)
            modulusSq = Math.pow(operand[0][0], 2) + Math.pow(operand[0][1], 2);
        ...
        case 'ln':
            ...
            let theta = Math.atan2(operand[0][1], operand[0][0]);
        ...
        case 'summation':
            if (operand[0][0] === 'equation'
                ...
                && operand[1][0] === 'natural') {
                let sum = [0, 0];
                let vari = operand[0][1];
                let min = operand[0][2][1];
                let max = operand[1][1];
...
export function evaluateVariable(variable, lookupTable = new Object()) {
    ...
    let keys = Object.keys(lookupTable);
    let is_included = false;
    for (let v of keys) {
        if (JSON.stringify(v) === JSON.stringify(varname)) {
            is_included = true;
...
export function fracOfPow2PowOfFrac(tree) {
    ...
    for (let [k, subtree] of tree_entries) {
        if (k === 0) {
            continue;
        }
...
export function evalNumericValues_power(operand) {
    ...
    let expFrac_entries = expFrac.entries();
    for (let [k, obj] of expFrac_entries) {
        if (obj[0] === 'negative') {
...
export function findGCF(tree = null) {
    ...
    switch (operator) {
        ...
        case 'mulchain':
            ...
            for (let multerm of tree_1) {
                ...
                if (multerm[0] === 'mul') {
                    ...
                    let subCFArr_sym = subCFArr['sym'];
                    for (let symb of subCFArr_sym) {
                        let is_included = false;
        ...
        case 'inequality':
            ...
            for (let [k, term] of tree_1_entries) {
                ...
                if (k === 0) {
                    ...
                    let symFArr = [];
                    let subCFArr_sym = subCFArr['sym'];
                    let commonFactorArr_sym = commonFactorArr['sym'];
                    for (let symb of subCFArr_sym) {
                        
                        for (let symb_1 of commonFactorArr_sym) {
...
export function findVars(tree, option = false) {
    ...
    if (option === true) {
        ...
        for (let term of tree_2) {
            let subtree = ['addchain', 'mulchain'].includes(tree[0]) ? term[1] : term;
...
export function multFactor(tree, fterm, simplifyFrac = false) {
    ...
    if (tree[0] === 'fraction') {
        let idx = fterm[0] === 'div' ? 2 : 1;
        if (simplifyFrac && JSON.stringify(fterm[1]) === JSON.stringify(tree[3 - idx])) {
            tree[3 - idx] = ['natural', '1'];
        } else {
            tree[idx] = combine2ChainTree(fterm[1], tree[idx], 'mul');
        }
        tree = fracNegative(tree);
        tree = fracSimpInt(tree);
        tree = fracSimpVar(tree);
        
    } else if (tree[0] === 'addchain') {
        ...
        if (fterm[0] === 'mul') {
            tree = combine2ChainTree(fterm[1], tree, 'mul');
        } else {
...
export function termExists(term, tree, recursive = true, excPolyTerm = false) {
    ...
    if (recursive === false) {
        ...
        for (let subterm of tree_2) {
            let subtree;
            if (['addchain', 'mulchain'].includes(tree[0])) {
                subtree = subterm[1];
            } else {
                subtree = subterm;
            }
    ...
    for (let subtree of tree) {
        if (termExists(term, subtree, recursive, excPolyTerm)) {
            ...
            let subtree_11 = subtree.slice(1);
            let is_included = false;
    ...
    switch (tree[0]) {
        ...
        case 'interval':
            return tree.length !== 5 ? false
                : !['[', '('].includes(tree[1]) || !tree[4], [']', ')'].includes(tree[4]) ? false

  ...
  switch (tree[0]) {
    ...
    case 'inequality':
      ...
      if (tree[2] === 'gt' || tree[2] === 'ge') {
        ...
        for (const [k, subtree] of tree_2.entries()) {
          k === tree.length - 2
          ...
          if (['equation', 'inequality', 'neq', 'ratio', 'orchain'].includes(evalresult[0])) {
            ...
            for (const evalside of evalresult_1) {
              if (!isFinite(evalside[0]) || isNaN(evalside[0]) ||
                                !isFinite(evalside[1]) || isNaN(evalside[1])) { return tree }
            }
...
export function evaluateOperation (operator, operand, seed = null, lookupTable = new Object()) {
  switch (operator) {
    ...
    case 'rdecimal':
      ...
      let num
      operand[1] === ''
        ? num = parseFloat(operand[1] + operand[2])
    ...
    case 'log':
      ...
      typeof operand[1] !== 'undefined'
        ? newOperand.push(evaluateOperation('ln', [operand[1]], seed, lookupTable))
...
export function combine2ChainTree (tree1, tree2, subtype = null) {
  ...
  const t1Arr = chainTree2Array(tree1, subtype)
  const t2Arr = chainTree2Array(tree2, subtype)

  const tArr = t1Arr.concat(t2Arr)
...
export function evalNumericValues (tree) {
  ...
  switch (operator) {
    ...
    case 'negative':
      const subtree = evalNumericValues(operand[0])
  ...
  switch (tree[0]) {
    ...
    case 'interval':
      return tree.length !== 5
        ? false
        : !['[', '('].includes(tree[1]) || !tree[4], [']', ')'].includes(tree[4])
        ? false
